...
%     [rate_outL, pw_outL, ppfpchL] = check_timing_parameters(num_selected, rate, pw, ipg);
    rate_outL = rate; pw_outL = pw; ppfpchL = ipg;
...
%         Total_Stimulation_Rate=14400; thr=Total_Stimulation_Rate; % This is the maximum stimulation rate supported by CIC4 in STD mode
        Total_Stimulation_Rate=28800; thr=Total_Stimulation_Rate; % This is the maximum stimulation rate supported by CIC4 in STD mode
        % 20180409 double the Total_Stimulation_Rate, for simulation
        % purpose
        ...
        rate = floor(rate);
        pw = floor(pw);       
        pulses_per_frame_per_channel=round((8.0*rate)/1000); %floor(0.5+floor((8.0*rate_set)/1000));
        ...
        if (pd1<pd2)
            ...
            while (pd1<pd2)
                ratetest = ratetest-1;
                pulses_per_frame_per_channel=round((8.0*ratetest)/1000); %floor(0.5+floor((8.0*rate_set)/1000));
                pulses_per_frame= num_selected*pulses_per_frame_per_channel;
