...
% 		time_label = 'Time (us)';
% 	end;
		time_scale = 1000000; % Meng 20220322 edit
		time_label = 'time (s)';
...
% 	set(u.h_figure, 'KeyPressFcn', s);
    	...
    	set(gcf, 'KeyPressFcn', s);

	%u.h_axes   = axes; % 1220/2017  by meng
...
t(end) = [];
    ...
    h = c + m / 255 ;% / max_mag;					% Line height is proportional to magnitude.
	y = [c; h; z];
	y = y(:);								% column vector

	hdl = line(x, y, 'Color', 'black');  %
%     hdl = line(x, y, 'Color', rand(1,3)*0.7); %hold on; % 20171220
