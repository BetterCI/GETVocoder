...
% sat:          Logical FTM indicating the values that were above SaturationLevel.
       ...
       % Scale the input between BaseLevel and SaturationLevel:
        r = (u - p.BaseLevel)/(p.SaturationLevel - p.BaseLevel);% u = r*(p.SaturationLevel - p.BaseLevel)+p.BaseLevel
        ...
        v = log(1 + p.lgf_alpha * r) / log(1 + p.lgf_alpha); % r = (exp(v)*(1 + p.lgf_alpha)-1)/p.lgf_alpha
